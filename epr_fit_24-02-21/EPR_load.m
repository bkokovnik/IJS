...
% ... added apply to all feature
        otherwise
            ...
            if autoApplyOptions && ~isempty(savedAnswers)
                ...
                choice = questdlg('Automatically apply these options to all following files?', ...
...
%     data{ind}.exp = exp;
    ...
    if isfield(exp,'Temperature')
        ...
        % Regex pattern to match temperatures with a decimal comma
        expression_half_integer = '_([\d]+)p([\d])K';
        expression_integer = '_([\d]+)K';
    
        % Check if the filename matches the half-integer pattern
        temperature_half_integer = regexp(fname, expression_half_integer, 'tokens');
    
        if numel(temperature_half_integer) == 1
            % Convert the matched pattern to a numeric value
            temp(ind) = str2double([temperature_half_integer{1}{1}, '.', temperature_half_integer{1}{2}]);
            exp.Temperature = temp(ind);
        else
            % Fallback to integer temperature pattern
            temperature_integer = regexp(fname, expression_integer, 'tokens');
            if numel(temperature_integer) == 1
                temp(ind) = str2double(temperature_integer{1}{1});
                exp.Temperature = temp(ind);
            else
                temp(ind) = 295; % Default to room temperature (295 K)
            end
        end
    end

...
switch name
    ...
    case 'dpVoigt'      
        funstr = '-a#*(alpha#*16/pi*w#*(x-xc#)/(4*(x-xc#)^2+w#^2)^2+2*(1-alpha#)*4*log(2)/(sqrt(pi)*w#^3)*(x-xc#)*exp(-(x-xc#)^2*4*log(2)/w#^2))';
...
% First Article corrected!!!
    ...
    otherwise
        funstr = {'dLorentz','dDyson','dGauss','Lorentz','Gauss','Dyson','pVoigt','dpVoigt','CoupledLorentzians','twoDLorDiffW','rubyRA','rubyR','qxx','s*x','y0'}; % !!! s*x and y0 must be last two!!!
